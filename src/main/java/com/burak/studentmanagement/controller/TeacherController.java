...
public class TeacherController {
	...
	public String showTeacherCourseDetails(@PathVariable("teacherId") int teacherId,
		...
		if (students.size() != 0) {
			List<Assignment> assignments = studentCourseDetailsService
					.findByStudentAndCourseId(students.get(0).getId(), courseId).getAssignments();
			...
			for (Student student : students) {
				gradeDetailsList.add(studentCourseDetailsService.findByStudentAndCourseId(student.getId(), courseId)
						.getGradeDetails());
			}
	...
	public String editGradesForm(@PathVariable("teacherId") int teacherId, @PathVariable("courseId") int courseId,
		...
		List<GradeDetails> gradeDetailsList = new ArrayList<>();
		for (Student student : students) {
			gradeDetailsList.add(
					studentCourseDetailsService.findByStudentAndCourseId(student.getId(), courseId).getGradeDetails());
		}
	...
	public String modifyGrades(@ModelAttribute GradeDetails gradeDetails,
		...
		// studentCourseDetailsService.findByStudentAndCourseId(gradeDetailsId,
		// courseId);
		StudentCourseDetails studentCourseDetails = gradeDetailsService.findById(gradeDetailsId)
		...
		for (Student student : students) {
			AssignmentDetails studentCourseAssignmentDetail = assignmentDetailsService
					.findByAssignmentAndStudentCourseDetailsId(assignmentId,
							studentCourseDetailsService.findByStudentAndCourseId(student.getId(), courseId).getId());
